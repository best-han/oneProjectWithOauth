...
public class RedisClientDetailsService extends JdbcClientDetailsService {
     ...
     */
    private static final String CACHE_CLIENT_KEY = "client:oss:client_details";
    ...
    private ClientDetails cacheAndGetClient(String clientId) {
        ...
        if (clientDetails != null) {// 写入redis缓存
            ...
            log.info("缓存oss:clientId:{},{}", clientId, clientDetails);
        }
    ...
    public void loadAllClientToCache() {
        ...
        log.info("将oss:oauth_client_details全表刷入redis");

        List<ClientDetails> list = super.listClientDetails();

        if (CollectionUtils.isEmpty(list)) {
            log.error("oss:oauth_client_details表数据为空，请检查");
            return;
        }
